...
### Task 20: Icons in 16x16

Laeuft **nach** Aufgabe 19 — beide fassen `Icons.swift` und `IconEditorView` an,
und ein animiertes 16x16-Icon ist die Kreuzung aus beiden.

Heute ist ueberall 8x8 fest eingebaut (`pixel.count == 64`). Das Display ist aber
16 hoch: ein 16x16-Icon fuellt die volle Hoehe und belegt 16 der 52 Spalten, dem
Text bleiben 36 statt 44. Das ist der Gewinn — ein Icon, das nicht wie eine
Briefmarke danebenklebt.

**Vorher zu klaeren, am Geraet:** `docs/tc002-protokoll.md` §4.2 fuehrt als offene
Frage, ob die Bildgroesse beschraenkt ist. Die gesamte AWTRIX-Welt arbeitet mit
8x8. **Wenn die Uhr 16x16 nicht annimmt, ist diese Aufgabe gegenstandslos** — der
Nutzer probiert das aus, bevor hier jemand anfaengt.

**Files:**
- Modify: `Sources/TC002Core/Icons.swift`
- Modify: `Sources/TC002App/IconEditorView.swift`, `SendenView.swift`, `HilfeView.swift`
- Test: `Tests/TC002CoreTests/IconsTests.swift`

- [ ] **Step 1: Der Kern lernt zwei Groessen**

`sichern` und `bilder(fuer:)` nehmen die Kantenlaenge entgegen statt 8 anzunehmen;
erlaubt sind **8 und 16**, alles andere wirft. Die Pixelzahl ist `kante * kante`.
Die vorhandenen Aufrufe bekommen `kante: 8` als Vorgabe, damit die bestehenden
Tests unveraendert gelten.

`Icon` traegt die Groesse mit, damit die Ansicht sie nicht raten muss:

```swift
    /// 8 oder 16. Aus der Bilddatei gelesen, nicht aus dem Namen.
    public var kante: Int
```

Beim Einlesen in `alle()` ueber `CGImageSourceCopyPropertiesAtIndex` bestimmen —
**nicht** ueber eine Namenskonvention, die beim Umbenennen zerbricht.

- [ ] **Step 2: Tests**

```swift
func testSechzehnerIconUeberlebtDenRundlauf() throws {
    let eigen = temp()
    try FileManager.default.createDirectory(at: eigen, withIntermediateDirectories: true)
    let sammlung = Iconsammlung(schreibordner: eigen)
    var p = [String?](repeating: nil, count: 256)
    p[0] = "#FF0000"; p[255] = "#00FF66"
    let icon = try sammlung.sichern(nummer: "gross", name: "Gross", pixel: p, kante: 16)
    XCTAssertEqual(icon.kante, 16)
    let zurueck = try sammlung.bilder(fuer: icon)[0]
    XCTAssertEqual(zurueck.count, 256)
    XCTAssertEqual(zurueck[0], "#FF0000", "oben links bleibt oben links")
    XCTAssertEqual(zurueck[255], "#00FF66")
}

func testNurAchtUndSechzehn() {
    let sammlung = Iconsammlung(schreibordner: temp())
    XCTAssertThrowsError(try sammlung.sichern(nummer: "x", name: "x",
                                              pixel: [String?](repeating: nil, count: 144), kante: 12))
}

func testVerdoppelnMachtAusJedemPixelEinenViererblock() {
    var klein = [String?](repeating: nil, count: 64)
    klein[0] = "#FF0000"                      // oben links
    let gross = Iconsammlung.verdoppeln(klein)
    XCTAssertEqual(gross.count, 256)
    for (x, y) in [(0, 0), (1, 0), (0, 1), (1, 1)] {
        XCTAssertEqual(gross[y * 16 + x], "#FF0000", "der Viererblock oben links")
    }
    XCTAssertNil(gross[2], "daneben bleibt es leer")
}
```
...
```swift
    /// Zieht ein 8x8-Raster auf 16x16: jeder Pixel wird ein Viererblock. Bewusst
    /// ohne Glaettung — bei sechzehn Pixeln Hoehe waere jede Zwischenfarbe Matsch.
    /// Das Ergebnis ist ein Ausgangspunkt zum Nachbessern, kein fertiges Icon.
    public static func verdoppeln(_ pixel: [String?]) -> [String?]
```

- [ ] **Step 4: Der Editor kann beides**

Das Raster im Icon-Editor richtet sich nach der Kantenlaenge des geoeffneten Icons;
fuer ein neues waehlt man sie (8 oder 16). Bei einem geoeffneten 8x8-Icon erscheint
**„Auf 16x16 vergroessern"**: verdoppelt das Bild, stellt das Raster auf 16 und
setzt die Nummer auf `<nummer>-16` vor, damit das Original erhalten bleibt. Danach
laesst sich frei weitermalen — **das ist der eigentliche Sinn**, das blosse
Verdoppeln koennte die App auch beim Senden selbst erledigen.

Bei animierten Icons (Aufgabe 19) gilt das fuer **alle** Einzelbilder.

- [ ] **Step 5: Senden**

Die Icon-Liste zeigt die Kantenlaenge an. Ein 16x16-Icon wird bei `y: 0` gesetzt
statt bei `y: 4` — es fuellt die Hoehe —, und dem Text bleiben die Spalten ab 18
statt ab 10. Die Warnung „passt nicht" rechnet damit.

Gibt es zu einem gewaehlten 8x8-Icon ein gleichnamiges 16x16 (`<nummer>-16`), biete
es zur Wahl an: **„klein (8x8)" / „gross (16x16)"**. Fehlt die grosse Fassung,
erscheint die Wahl nicht.

- [ ] **Step 6: Hilfe, bauen, Commit**

Hilfe, Abschnitt „Icons": beide Groessen, das Vergroessern als Ausgangspunkt, und
dass 16x16 die volle Hoehe fuellt und dem Text Platz nimmt. Dazu der ehrliche
Hinweis, dass 16x16 am Geraet erprobt gehoert.

```bash
swift test && ./build.sh
git add <die von dir angelegten/geaenderten Dateien einzeln> && git commit -m "feat: Icons in 16x16"
```
