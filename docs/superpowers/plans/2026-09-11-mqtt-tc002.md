...
struct AnzeigenView: View {
    ...
    var body: some View {
        ...
        .task {
            ...
            geladen = true
            guard let uhr = zustand.aktiveUhr else { return }
            if let k = try? Geraet(host: uhr.host).konfiguration(),
...
```bash
git add <die von dir angelegten/geaenderten Dateien einzeln> && git commit -m "docs: README und Arbeitsregeln, Abnahme am Geraet"
```
