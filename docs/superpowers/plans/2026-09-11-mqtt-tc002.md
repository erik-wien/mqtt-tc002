...
### Task 21: Bildersammlung

Unter „Malen" laesst sich heute genau ein Bild malen, und es ueberlebt den
Neustart. Diese Aufgabe macht daraus eine Sammlung: mehrere Bilder unter Namen
sichern, wieder laden, loeschen — wie die Icon-Bibliothek, nur fuer ganze
52×16-Bilder.

Laeuft **nach** der Runde, die gerade die Senden-Ansicht umbaut und das zuletzt
gemalte Bild sichert. Das zuletzt gemalte Bild bleibt, wie es ist — es ist der
Arbeitsstand, die Sammlung ist die Ablage.

**Files:**
- Create: `Sources/TC002Core/Bildersammlung.swift`, `Tests/TC002CoreTests/BildersammlungTests.swift`
- Create: `Sources/TC002App/BilderView.swift`
- Modify: `Sources/TC002App/MalenView.swift`, `HilfeView.swift`

**Interfaces:**
- Consumes: `Pixelfeld`, `Pixelfeld.punkteRoh`, `Pixelfeld.init?(breite:hoehe:punkte:)`
- Produces: `Bildersammlung` mit `alle() -> [Gemaltes]`, `sichern(name:feld:) throws -> Gemaltes`,
  `laden(_:) throws -> Pixelfeld`, `loeschen(_:) throws`; `Gemaltes` (`name`, `datei`)
...
`Bildersammlung` liegt in `~/Library/Application Support/MQTT-TC002/Bilder` — derselbe
Ort wie die eigenen Icons, eigener Unterordner. Gesichert wird als **GIF in 52×16**,
nicht als JSON: So sind die Bilder auch ausserhalb der App zu sehen, und der Weg
ueber `CGImageDestination` steht schon, weil die Icons ihn gehen.

Der Dateiname ist der Name, bereinigt um alles, was in Dateinamen nichts verloren hat
(Schraegstriche, Doppelpunkte); der angezeigte Name kommt aus einer `namen.json`
daneben, wie bei den Icons. Gleicher Name **ersetzt**.

Das Lesen ist dasselbe Verfahren wie bei den Icons, nur 52×16 statt 8×8 — zieh die
gemeinsame Umrechnung heraus, statt sie ein zweites Mal zu schreiben. **Achtung:**
`CGContext` faengt unten links an, das Raster oben links. Genau daran ist hier schon
zweimal etwas auf dem Kopf gestanden, deshalb pruefen die Tests beide Ecken.

- [ ] **Step 3: Die Ansicht**

In `MalenView` ein Knopf **„Bilder"**, der ein Blatt oeffnet — gleiche Bauweise wie
die Blaetter fuer Ziel und Icon, damit sich alles gleich anfuehlt. Darin:

- die gesicherten Bilder als Vorschaubilder mit Namen,
- ein Bild anklicken laedt es ins Raster (**mit Rueckfrage**, wenn das aktuelle Bild
  nicht leer ist — sonst ist eine halbe Stunde Arbeit mit einem Klick weg),
- „Loeschen" je Bild,
- ein Feld fuer den Namen und „Sichern", das das aktuelle Raster ablegt.

- [ ] **Step 4: Hilfe, bauen, Commit**

Hilfe, Abschnitt „Malen": die Sammlung, wo die Dateien liegen, und dass gleicher
Name ersetzt.

```bash
swift test && ./build.sh
git add <die von dir angelegten/geaenderten Dateien einzeln> && git commit -m "feat: Bildersammlung unter Malen"
```

---

### Task 22: Bilddateien einlesen

Neue Icons kommen heute nur ueber LaMetric-Nummern herein, Bilder gar nicht. Wer
einen Ordner voller GIFs hat, kommt nicht an sie heran. Diese Aufgabe laesst beide
Sammlungen Dateien aufnehmen — GIF, PNG und JPEG.

Laeuft **nach** Aufgabe 21, weil sie deren `Bildersammlung` mitbenutzt.

**Files:**
- Modify: `Sources/TC002Core/Icons.swift`, `Sources/TC002Core/Bildersammlung.swift`
- Modify: `Sources/TC002App/IconEditorView.swift`, `BilderView.swift`, `HilfeView.swift`
- Test: `Tests/TC002CoreTests/IconsTests.swift`, `BildersammlungTests.swift`

**Interfaces:**
- Produces: `Bildraster.lesen(_:breite:hoehe:) throws -> [[String?]]`;
  `Iconsammlung.einfuegen(datei:nummer:name:) throws -> Icon`;
  `Bildersammlung.einfuegen(datei:name:) throws -> Gemaltes`

- [ ] **Step 1: Das Umrechnen an eine Stelle**

Icons lesen heute 8×8, die Bildersammlung 52×16, und beide tun dasselbe. Zieh es
zu einem gemeinsamen Baustein zusammen, der **alle** Einzelbilder einer Datei auf
eine gewuenschte Groesse rechnet:
...
`Iconsammlung.bilder(fuer:)` und das Laden der `Bildersammlung` gehen danach ueber
diesen Weg. Der **Ursprung** bleibt die Falle: `CGContext` faengt unten links an,
das Raster oben links. Hier ist schon zweimal etwas auf dem Kopf gestanden.

- [ ] **Step 2: Tests**

```swift
func testFremdeGroesseWirdGerechnet() throws {
    // 16x16 mit einem roten Pixel oben links -> auf 8x8 bleibt oben links rot
...
`schreibeTestbild` legst du als Hilfe in der Testdatei an — ueber
`CGImageDestination`, wie es `sichern` tut.

- [ ] **Step 3: Einfuegen in beide Sammlungen**

```swift
    /// Nimmt eine Bilddatei in die Sammlung auf, auf 8×8 gerechnet. Animierte GIFs
    /// behalten ihre Einzelbilder.
    @discardableResult
    public func einfuegen(datei: URL, nummer: String, name: String) throws -> Icon
```

und das Gegenstueck in `Bildersammlung` fuer 52×16. Beide gehen ueber
`Bildraster.lesen` und dann ueber das vorhandene `sichern` — **die Quelldatei wird
nicht kopiert**, sondern neu geschrieben, damit in der Sammlung ausschliesslich
Dateien in der richtigen Groesse liegen.

Eine Datei, die gar kein Bild ist, ergibt einen Fehler mit verstaendlicher Meldung,
keinen Absturz und keinen leeren Eintrag.

- [ ] **Step 4: Die Bedienung**

In beiden Blaettern ein Knopf **„Datei einlesen…"** mit `.fileImporter`, erlaubte
Typen `.gif`, `.png`, `.jpeg`. Nach der Wahl ein kleiner Dialog fuer Nummer und
Namen (bei Icons) beziehungsweise den Namen (bei Bildern), mit dem Dateinamen als
Vorschlag.

**Weis darauf hin, wenn gerechnet wurde:** Hat die Datei nicht schon die
Zielgroesse, sag es in einem Satz — „Das Bild wurde von 64×64 auf 8×8 gerechnet."
Wer ein grosses Bild einliest und Matsch bekommt, soll wissen, warum.

Mehrfachauswahl waere schoen, ist aber nicht verlangt: eine Datei je Durchgang
genuegt.

- [ ] **Step 5: Hilfe, bauen, Commit**

Hilfe: in „Icons" und „Malen" je ein Absatz — welche Formate, dass gerechnet wird
und wie, und dass animierte GIFs ihre Einzelbilder behalten.

```bash
swift test && ./build.sh
git add <die von dir angelegten/geaenderten Dateien einzeln> && git commit -m "feat: Bilddateien in Icons und Bilder einlesen"
```
