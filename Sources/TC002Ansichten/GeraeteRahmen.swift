...
/// Seitenverhaeltnis mit (52:16 = 3,25); das Feld ist minimal breiter
/// (584:177 = 3,30). Deshalb wird `inhalt` an der Feldhoehe ausgerichtet und
/// horizontal zentriert — der schmale schwarze Rest links und rechts faellt
/// nicht auf, und die Pixel bleiben quadratisch. Das ruehrt an nichts in
/// `Meldungsbau`, das die tatsaechliche Nutzlast erzeugt; hier geht es allein
/// um Bildschirmgeometrie.
public struct GeraeteRahmen<Inhalt: View>: View {
    let breite: Double
    let hoehe: Double
    @ViewBuilder let inhalt: Inhalt

    public init(breite: Double, hoehe: Double, @ViewBuilder inhalt: () -> Inhalt) {
        self.breite = breite
        self.hoehe = hoehe
        self.inhalt = inhalt()
    }
