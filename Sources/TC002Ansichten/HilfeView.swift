    ...
    var bausteine: [Hilfebaustein] {
        ...
        case .senden:
            return [
                ...
                + [
                    ...
                    .absatz("Zwei Entscheidungen, zwei Griffe: **welche Uhr man ansieht** steht mittig in der Werkzeugleiste als Menü mit ihrem Namen — daran hängen Vorschau, Geräterahmen und die fünf Blöcke —, und darunter sagt eine Punktreihe, wie viele Uhren es gibt und die wievielte das ist. Anklicken wechselt, Wischen über der Vorschau ebenso."),
                    .absatz("**Wohin gesendet wird**, nennt der Knopf oben rechts über der Vorschau — etwa „an: Küche“, „an alle Uhren (3)“ oder „an 2 Uhren“. Ein Druck öffnet ein Blatt mit einer Zeile je eingerichteter Uhr: Name, darunter das Präfix oder „HTTP“, bei MQTT-Uhren dazu derselbe Verbindungsstand wie unter „Einstellungen“, und ein Haken zum An- und Abwählen; „Alle“ und „Nur die angesehene“ wählen mit einem Klick, „Schließen“ beendet die Auswahl."),
                    ...
                    .absatz("Einen eigenen Sendeknopf gibt es nicht: Die Eingabetaste schickt, und am rechten Rand des Feldes steht dafür ein ⏎, das man auch anklicken kann. Es erscheint, sobald etwas im Feld steht, und weicht während des Sendens einem Fortschrittsdreher. Links daneben leert ein ⊗ das Feld. Geht dabei etwas schief — die Uhr nicht erreichbar, die Uhr weist die Anzeige ab, falsches Broker-Kennwort, Broker nicht erreichbar, Zeitüberschreitung, unlesbare Icondatei —, erscheint ein Hinweisfenster mit dem Grund; bei mehreren Zieluhren eine Zeile je betroffener Uhr, die übrigen werden trotzdem beliefert."),
        ...
        case .editor:
            return [
                    ...
                    .absatz("„Verzögerung“ gilt für jedes Einzelbild gleich, in Sekunden. Abgespielt wird unter der Leinwand: Das runde Zeichen dort läuft den Streifen probeweise in Schleife durch, ohne dass vorher gesichert werden muss, und wird zur Pause, solange es läuft — angehalten bleibt das gerade gezeigte Einzelbild stehen. Bei nur einem Einzelbild steht es gar nicht erst da. Die Uhr spielt animierte GIFs ab, nicht nur deren erstes Einzelbild — am Gerät bestätigt (Hilfe → Gerätereferenz, §4.2)."),
                    ...
                    .absatz("Darunter stehen alle drei Bestände in einer Liste, jeder Eintrag mit seiner Größe als Merkmal und, wo es eine gibt, mit seiner Nummer. Das Suchfeld grenzt nach Name und Nummer ein, die Leiste darunter nach Größe (8 × 8, 16 × 16, 16 × 52) und darauf, ob sich etwas bewegt — dieselbe Leiste wie im Auswahlblatt für Icons. Ein Klick auf eine Zeile lädt sie mit allen Einzelbildern zurück auf die Leinwand — mit Rückfrage, wenn dort Ungesichertes steht. Das Papierkorb-Symbol in der Zeile löscht, mit Rückfrage, die den Namen nennt; dasselbe bietet „Löschen“ im Kontextmenü."),
