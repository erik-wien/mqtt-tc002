...
extension Geraetetyp {
    ...
    ///
    /// **Die Werksfirmware hat sechzehn Zeilen, NG hat acht.** Ein 16×16-Icon
    /// ist dort nicht bloss gross, es geht gar nicht: Die Leinwand fuer Icons
    /// ist 32×8, und ein GIF, dessen erstes Bild hoeher ist, **spielt
    /// ueberhaupt nicht** (§8) — ohne jede Meldung. Das ist eine Eigenschaft
    /// des Geraets und kein Fehler, der sich glattbuegeln liesse; gesagt gehoert
    /// er trotzdem.
    public var iconKanten: [Int] {
        switch self {
        case .tc002: return [8, 16]
        case .awtrixNG: return [8]
        }
    }
