...
public struct Ablageort: Sendable, Equatable {
    ...
    public enum Bestand: String, CaseIterable, Sendable {
        ...
        case bilder = "Bilder"
        case slots = "Slots"
        /// Der Sendeverlauf. Eine Datei je Installation,
    ...
    ///
    /// Ausdruecklich nicht die im Behaelter. Diese Methode wird aus
    /// `Ablageort.gemeinsam` heraus gerufen, dem ersten Zugriff ueberhaupt —
    /// legte sie am *wirksamen* Ort an, also im Behaelter, sobald der Abgleich
    /// laeuft, taeten das zwei Geraete unabhaengig voneinander, jedes bevor
    /// der Behaelter bei ihm angekommen ist. `createDirectory` sieht dann
    /// nichts, legt an — und iCloud macht daraus `Icons`, `Icons 2`, `Bilder`,
    /// `Bilder 2` und so fort: acht Ordner statt vier.
    ///
...
extension Ablageort {
    ...
    public static func umschalten(_ an: Bool,
        ...
        // acht daraus. Blockiert — steht hier richtig, `umschalten` laeuft
